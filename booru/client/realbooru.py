...
class Realbooru(ClientSessionMixin):
    """Realbooru wrapper
    ...
    async def search(
        ...
        raw_data = await request(self.http_session, site=Booru.realbooru, params_x=self.specs, block=block)
    ...
    async def find_tags(self, query: str) -> Union[list, str, None]:
        ...
        """
        try:
            data = await  request_wildcard(self.http_session, site=Booru.realbooru_wildcard, query=query)
