...
async def request(
    ...
    async with http_session.get(site, params=params_x, headers=ua) as resp:
...
async def request_wildcard(http_session: aiohttp.ClientSession, site: str, query: str) -> Union[list, None]:
    """Fetch wildcard
    ...
    async with http_session.get(f"{site}{query}*{sorting}") as resp:
